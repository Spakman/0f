...
# clearing excludes).
down_sync_output=`rsync -i --exclude-from ${excludes_file} --delete -a -u -X -A -e "ssh -p3237 -i $HOME/.ssh/id_rsa" fingers.today@fingers.today:./fingers.today/pages/ ${root}/pages/`
...
# Sync up
rsync --delete -a -u -X -A -e "ssh -p3237 -i $HOME/.ssh/id_rsa" ${root}/pages/ ${domain}@${domain}:./${domain}/pages/
